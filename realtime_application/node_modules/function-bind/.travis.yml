...
script:
  - 'if [ -n "${PRETEST-}" ]; then npm ml_component pretest ; fi'
  - 'if [ -n "${POSTTEST-}" ]; then npm ml_component posttest ; fi'
  - 'if [ -n "${COVERAGE-}" ]; then npm ml_component coverage ; fi'
  - 'if [ -n "${TEST-}" ]; then npm ml_component tests-only ; fi'
sudo: false
env:
  - TEST=true
matrix:
